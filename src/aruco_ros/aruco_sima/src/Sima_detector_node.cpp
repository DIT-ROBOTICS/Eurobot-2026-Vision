...
public:
    SimaDetectorNode() : Node("sima_detector") {
        ...
        // 減少標籤內部格子的採樣邊距，讓它更靠近格子中心，避開模糊邊界
        detector_params_->markerBorderBits = 1; 
        // 提高解碼時對錯誤位元的容忍（16h5 很脆弱，通常設為 0 或 1）
        detector_params_->errorCorrectionRate = 0.6;
        // 增加這個值可以濾掉那些「看起來不夠方」的雜訊。
        ...
        subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/camera/camera/color/image_raw", 10, std::bind(&SimaDetectorNode::image_callback, this, std::placeholders::_1));
        info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
            "/camera/camera/color/camera_info", 10, std::bind(&SimaDetectorNode::camera_info_callback, this, std::placeholders::_1));
...
private:
    ...
    void image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        for (size_t i = 0; i < ids.size(); i++) {
            ...
            cv::Vec3d rvec, tvec;
            if (cv::solvePnP(objPoints, corners[i], camera_matrix_, dist_coeffs_, rvec, tvec)) {
                // 2. 轉換為四元數
                cv::Mat R;
                cv::Rodrigues(rvec, R);
